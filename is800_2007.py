...
class IS800_2007(object):
    ...
    def cl_7_5_1_2_equivalent_slenderness_ratio_of_truss_compression_members_loaded_one_leg(length, r_min, b1, b2, t, f_y, bolt_no=2, fixity='Fixed'):
        ...
        Args:
            length (float): Actual length of the member in mm.
            r_min (float): Minimum radius of gyration of the section (angle).
            b1 (float): First leg of the section (angle).
            b2 (float): Second leg of the section (angle).
            t (float): Thickness of the section.
            f_y (float): Yield strength of the material.
            bolt_no (int, optional): Number of bolts in the connection (default: 2).
            fixity (str, optional): End condition ('Fixed', 'Hinged', 'Partial'). Default is 'Fixed'.

        Returns:
            list: [equivalent_slenderness_ratio, lambda_vv, lambda_psi, k1, k2, k3]
        """
        e = math.sqrt(250 / f_y)
        E = 2 * 10 ** 5
        ...
        if fixity in fixity_params:
            k1, k2, k3 = fixity_params[fixity]
        elif fixity == 'Partial':
            fixed_values = IS800_2007.cl_7_5_1_2_equivalent_slenderness_ratio_of_truss_compression_members_loaded_one_leg(length, r_min, b1, b2, t, f_y, bolt_no, 'Fixed')
            hinged_values = IS800_2007.cl_7_5_1_2_equivalent_slenderness_ratio_of_truss_compression_members_loaded_one_leg(length, r_min, b1, b2, t, f_y, bolt_no, 'Hinged')
            k1 = (fixed_values[3] + hinged_values[3]) / 2
            k2 = (fixed_values[4] + hinged_values[4]) / 2
            k3 = (fixed_values[5] + hinged_values[5]) / 2
        else:
            raise ValueError("Invalid fixity type. Must be 'Fixed', 'Hinged', or 'Partial'.")
